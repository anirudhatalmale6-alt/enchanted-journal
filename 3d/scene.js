...
const PH = PW * 1.337;            // the rectified cover's ratio
...
scene.add(new THREE.AmbientLight(0x4a2218, 0.30));
...
for (let i = 0; i < 9; i++) {
  ...
  const r = 0.20 + ((i * 7) % 5) * 0.055;
  g.position.set(Math.cos(a) * r, 0.0009, Math.sin(a) * r * 0.72 + 0.05);
  g.rotation.y = a * 1.7;
  scene.add(g);
}
...
const dustMat = new THREE.ShaderMaterial({
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      gl_FragColor = vec4(c.rgb, c.a * (0.10 + 0.90 * vTwinkle));
...
function step(dt) {
  ...
  for (const p of petals) {
    const d = p.userData;
    d.y -= d.fall * dt;
    const overBook = Math.abs(p.position.x) < 0.24 && Math.abs(p.position.z) < 0.17;
    if (d.y < (overBook ? 0.062 : 0.004)) {
      d.y = 0.95 + Math.random() * 0.25;
      d.x = (Math.random() - 0.5) * 1.15;
      d.z = -0.55 + Math.random() * 0.85;
    }
    p.position.set(
